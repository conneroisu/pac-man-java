...
public final class GhostTargetingTest {
  ...
  public static void main(String[] args) {
    System.out.println("Starting Ghost Targeting Test...");
    ...
    for (Actor ghost : ghosts) {
      ...
      System.out.println("Found ghost: " + className);
      
      if (className.equals("Blinky")) {
        blinky = ghost;
      } else if (className.equals("Pinky")) {
        pinky = ghost;
      } else if (className.equals("Inky")) {
        inky = ghost;
      } else if (className.equals("Clyde")) {
        clyde = ghost;
      }
    }
    ...
    if (blinky == null || pinky == null || inky == null || clyde == null) {
      System.err.println("ERROR: Could not identify all ghost types");
      return;
    }
  ...
  private static void testBlinkyTargeting(Actor blinky, Actor pacman, PacmanGame game) {
    ...
    verifyMovingToward(blinky, pacman.getCurrentLocation(), "Blinky", "Pacman's current position", game);
    
    // Move Pacman to a different location and verify Blinky follows
    placePacmanAtLocation(pacman, 20, 5, Direction.DOWN, game);
    verifyMovingToward(blinky, pacman.getCurrentLocation(), "Blinky", "Pacman's new position", game);
  }
  ...
  private static void testPinkyTargeting(Actor pinky, Actor pacman, PacmanGame game) {
    ...
    verifyMovingToward(pinky, expectedTarget, "Pinky", "4 tiles up AND 4 tiles left (UP direction bug)", game);
  }
  ...
  private static void testInkyTargeting(Actor inky, Actor blinky, Actor pacman, PacmanGame game) {
    ...
    Location expectedTarget = new Location(
  ...
  private static void testClydeTargeting(Actor clyde, Actor pacman, PacmanGame game) {
    ...
    verifyMovingToward(clyde, pacman.getCurrentLocation(), "Clyde", "Pacman's position (when far)", game);
    ...
    verifyNotMovingToward(clyde, pacman.getCurrentLocation(), "Clyde", "away from Pacman (when close)", game);
  }
  
  /**
   * Helper to place Pacman at a specific location.
   */
  private static void placePacmanAtLocation(Actor pacman, int row, int col, Direction dir, PacmanGame game) {
    ...
    System.out.println("Positioned Pacman at (" + row + ", " + col + ") facing " + dir);
  }
  
  /**
   * Helper to place a ghost at a specific location.
   */
  private static void placeGhostAtLocation(Actor ghost, int row, int col, Direction dir, PacmanGame game) {
    ...
    System.out.println("Positioned " + ghost.getClass().getSimpleName() + 
                     " at (" + row + ", " + col + ") facing " + dir);
  }
  
  /**
   * Verifies that a ghost is moving toward a target.
   */
  private static void verifyMovingToward(Actor ghost, Location target, String ghostName, 
                                        String targetDesc, PacmanGame game) {
    ...
    System.out.println(ghostName + " started at " + initialLoc + ", now at " + finalLoc);
    System.out.println("Distance to target " + targetDesc + " " + target + 
    ...
    if (finalDistance < initialDistance) {
      System.out.println("✓ PASS: " + ghostName + " is correctly moving toward " + targetDesc);
    } else {
      System.out.println("✗ FAIL: " + ghostName + " is NOT moving toward " + targetDesc);
    }
  }
   ...
   */
  private static void verifyNotMovingToward(Actor ghost, Location target, String ghostName, 
                                          String targetDesc, PacmanGame game) {
    Location initialLoc = ghost.getCurrentLocation();
    double initialDistance = calculateDistance(initialLoc, target);
    ...
    if (finalDistance >= initialDistance || Math.abs(finalDistance - initialDistance) < 0.1) {
      System.out.println("✓ PASS: " + ghostName + " is correctly NOT moving toward " + targetDesc);
    } else {
      System.out.println("✗ FAIL: " + ghostName + " IS moving toward " + targetDesc + 
