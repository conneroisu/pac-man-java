...
public final class PacmanPanel extends JPanel {
  ...
  /** Modulo check for odd. */
  private static final int MODULO_ODD_CHECK = 2;

  /** Result for odd modulo. */
  private static final int ODD_MODULO_RESULT = 1;
  ...
  private transient Location prev;
  ...
  public PacmanPanel(final PacmanGame game) {
    ...
    interval = MILLISECONDS_PER_SECOND / game.getFrameRate();
      Timer timer = new Timer(interval, new TimerCallback());
    timer.start();
    ...
    for (int i = 0; i < enemies.length; ++i) {
      ...
      if (ghost.getMode() == Mode.FRIGHTENED) {
        ...
        if (quarterSecondCount * FLASH_CYCLES >= count) {
          ...
          if (flag % MODULO_ODD_CHECK == ODD_MODULO_RESULT) {
            g.setColor(Color.WHITE);
          }
        }
      ...
      switch (dir) {
        ...
        case UP:
          ...
          break;
        case DOWN:
          eyeballY += shift + SINGLE_INCREMENT;
          break;
        default:
          // No action needed
      }
  ...
  private class TimerCallback implements ActionListener {
    ...
    public void actionPerformed(final ActionEvent e) {
      ...
      if (!curr.equals(prev)) {
        ...
        arcIncrement = (double) -MOUTH_OPEN_ANGLE / numUpdates;
        angle = MOUTH_OPEN_ANGLE + arcIncrement;
      } else if (angle <= ZERO_COMPARISON) {
