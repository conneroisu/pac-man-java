...
public abstract class ActorImpl implements Actor {
  ...
  private Location[] getNeighbors(Location tBAL) {
    ...
    if (tBAL.col() + 1 < maze.getNumColumns()
        && !maze.isWall(tBAL.row(), tBAL.col() + 1)
  ...
  private boolean scaredUtil(Descriptor desc) {
    if (getMode() == Mode.FRIGHTENED) {
      ...
      return true;
      // Set the next location to the location that the actor will arrive at after moving one
      // increment with the next direction
    }
    return false;
  }
  ...
  public void calculateNextCell(Descriptor d) {
    ...
    double smallestDistance =
        128.0; // Tracks the smallest distance between the Target Location and the neighboring cells
    // Get a singular distance from the neighbors array to the targetLocation that isn't null

    double
        calculatedDistanceTween; // Tracks the distance between the Target Location and the
                                 // neighboring cell being checked
    ...
    for (i = 0; i < neighbors.length; i++) {
      if (neighbors[i] != null) {
        ...
        if ((calculatedDistanceTween - smallestDistance < ERR
                && calculatedDistanceTween - smallestDistance > -ERR)
            || calculatedDistanceTween < smallestDistance) {
          // set the neighbor index to the current index if the distance is smaller
          if (calculatedDistanceTween - ERR < smallestDistance
  ...
  public void update(Descriptor description) {
    ...
    // if distance to center is within the bounds of the error arround zero, we have reached the
    // center
    ...
    if (previousLocation != currentLocation && !pastCenter) {
      calculateNextCell(description);
    }
  }
  ...
  /**
   * Determines the distance to the center of the cell through the difference between the current
   * position and the center of the current cell
   *
   * @return
   */
